...
```{r, echo=FALSE}
count <- la.census %>% filter(poverty.bool) %>% count() %>% .$n
iterations <- 1000
  
# Create a column of shuffle-able poverty bool values
LA.shuffle <- la.census %>%
  mutate(poverty = poverty.bool) %>%
  filter(population > 1000) %>%
  select(poverty.shuffle, population, sb.count)

sampleT <- function (n) {
  shuffle <- LA.poverty.shuffle %>% nrow() %>% sample()
  LA.shuffle$poverty <- LA.shuffle$poverty[shuffle]
  
  modelSampleA <- LA.shuffle %>% filter(poverty) %>%
    glm(sb.count ~ 1, data = ., offset=log(population), 
        family=quasipoisson)
  
  modelSampleB <- LA.shuffle %>% filter(!poverty) %>%
    glm(sb.count ~ 1, data = ., offset=log(population), 
        family=quasipoisson)
  # The average number of Starbucks per 1000 people
  lambdaA <- exp(modelSampleA$coefficients)*1000
  lambdaB <- exp(modelSampleB$coefficients)*1000
