...
```{r}
  countLocations <- function(polygon, locations, radius=1) {
    ...
    radius.at.lat <- earth.radius * cos(approx.lat * deg.torad)
    ...
    for (i in 1:nrow(locations)) {
      if (output[i] == FALSE) {
        ...
        polygon.long.range <- range(polygon$long) + (-1,1) * radius.long
        polygon.lat.range <- range(polygon$lat) + (-1, 1) * radius.lat
        in.int <- function(x,y) y[1] <= x & x <= y[2]
        nearby <- (int.int(long, polygon.long.range) &
        ...
        if (nearby) {
          ...
          output[i] <- (sum(point.in.polygon(long, lat, 
                                             polygon$long,
                                             polygon$lat)) > 0)
        }
