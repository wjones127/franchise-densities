...
la.census %<>% left_join(la.income, by='Geo_FIPS') %>%
  select(Geo_FIPS, population, pop.poverty, area.land)
...
#save(la.census, file='cached_data/census_with_counts.RData')
...
```{r, echo=FALSE, warning=FALSE}
qplot(y = rate.poverty, x = population / area.land,
      data = la.census, color = poverty) + 
  labs(x = 'Population / Land Area',
       y = 'Rate of Poverty',
       title = 'Population Density and Rate of Poverty in LA County') + 
  theme_bw()
```
...
LA.shuffle <- la.census %>%
  filter(population > 1000) %>%
  select(poverty, population, sb.count, area.land)

calcCoef <- function (df) {
  # Computes the average number of Starbucks per 1000 people per sq. mile
  model <- glm(sb.count ~ 1, data = df, offset = log(population / area.land),
               family = quasipoisson)
  return(exp(model$coefficients)*1000)
}
